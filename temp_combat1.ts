...
interface Unit {
  ...
  maxShield: number; // Maximum shield HP
  shieldRegen: number; // Shield regeneration per turn
  ap: number;
  maxAp: number;
  position: Position;
  facing: number;
  ...
  // Character stats (all default to 50 if not provided)
  ...
  armor?: Armor; // Equipped armor object (for stopping power calculations)
  acted: boolean;
  visible: boolean;
  stance: StanceType;
  grappleState: {
  ...
  shieldBar?: Phaser.GameObjects.Graphics; // Shield bar overlay
...
export class CombatScene extends Phaser.Scene {
  ...
  private spawnUnit(unitData: Partial<Unit> & { id: string; name: string; team: 'blue' | 'red' | 'green'; position: Position }): void {
    ...
    const unit: Unit = {
      ...
      dr: unitData.dr ?? 0,
      armor: undefined, // Will be set below if equippedArmor is provided
    ...
    // Load equipped armor if specified
    if ('equippedArmor' in unitData && unitData.equippedArmor) {
      const armorId = unitData.equippedArmor as string;
      const armor = getArmorById(armorId);
      if (armor) {
        unit.armor = armor;
        // Update DR from armor if not already set
        if (!unitData.dr) {
          unit.dr = armor.drPhysical;
        }
      }
    }
  ...
  private applyDamageTypeEffects(target: Unit, damageTypeId: string, damage: number, isCrit: boolean): void {
    ...
    if (damageTypeDef.stun?.enabled) {
      ...
      if (stunEffect.savingThrow) {
        ...
        if (stunRoll < 50) { // Failed saving throw
  ...
  private showFloatingDamage(x: number, y: number, damage: number, color: number): void {
    ...
    const damageText = this.add.text(x, y - 50, `-${finalDamage}`, {
      fontSize: '18px',
      color: colorHex,
      fontStyle: 'bold',
      stroke: '#000000',
...
private initiateCombat(spotter: Unit, spotted: Unit): void {
  ...
  EventBridge.emit('log-entry', {
    ...
    type: 'system',
    actor: 'System',
    message: `⚠️ CONTACT! ${spotter.name} spotted ${spotted.name}! Combat begins!`,
  });
...
private showContactAlert(spotter: Unit, spotted: Unit): void {
  ...
  const contactText = this.add.text(centerX, centerY, '⚠️ CONTACT! ⚠️', {
    fontSize: '64px',
    color: '#ff0000',
    fontStyle: 'bold',
    stroke: '#000000',
  ...
  this.units.forEach(unit => {
    if (unit.hp > 0) {
      ...
      EventBridge.emit('log-entry', {
        ...
        actor: unit.name,
        actorTeam: unit.team,
        message: `🎲 ${unit.name} initiative: ${roll} (INS: ${unit.ins || 50})`,
      });
    }
  });
...
private determineFirstTeam(): void {
  ...
  private tryAttackUnit(attackerId: string, targetId: string, callback?: () => void): void {
    ...
    this.fireVisualEffect(attacker, target, weapon, () => {
      ...
      // Apply armor DR to reduce damage
      let finalDamage = damage;
      let armorStopped = false;
      let armorPierced = false;
      if (didHit && damage > 0) {
        const armorResult = calculateDamageAfterArmor(damage, target.armor, weapon.penetrationMult ?? 1.0);
        finalDamage = armorResult.finalDamage;
        armorStopped = armorResult.stopped;
        armorPierced = armorResult.pierced;
      }
      ...
      if (didHit) {
        // Track hits and damage (use finalDamage after armor)
        this.combatStats.hits[attackerTeam]++;
        this.combatStats.damageByUnit[attacker.name].hits++;
        this.combatStats.damageByUnit[attacker.name].dealt += finalDamage;
        this.combatStats.damageByUnit[target.name].taken += finalDamage;
        this.combatStats.totalDamageDealt[attackerTeam] += finalDamage;
      ...
      if (didHit) {
        ...
        // Apply status effects based on damage type
        const damageSystemType = WEAPON_TO_DAMAGE_SYSTEM[attacker.weapon];
        if (damageSystemType && target.hp > 0) {
          this.applyDamageTypeEffects(target, damageSystemType, hpDamage, isCrit);
        }
        ...
        if (isCrit) {
          textContent = `${verbs.crit}! -${finalDamage}`;
          if (shieldDamage > 0) {
          ...
          textContent = `${verbs.graze} -${finalDamage}`;
          if (shieldDamage > 0) {
          ...
          textContent = `${verbs.hit} -${finalDamage}`;
          if (shieldDamage > 0) {
            textContent += ` (🛡️${shieldDamage}${shieldBroken ? ' BROKEN!' : ''})`;
          }
          fontSize = '16px';
  ...
  private runSingleAI(allies: Unit[], index: number): void {
    ...
    for (const unit of teamUnits) {
      if (unit.shieldRegen > 0 && unit.shield < unit.maxShield) {
        ...
        if (regenAmount > 0) {
          this.updateHealthBar(unit);
          this.emitToUI('combat-log', { message: `🛡️ ${unit.name}'s shield regenerates +${regenAmount} (${unit.shield}/${unit.maxShield})`, type: 'status' });
SHIELD_REGEN_INSERTION_POINT
        }
      }
    }
  ...
  private emitAllUnitsData(): void {
    const unitsData = Array.from(this.units.values()).map(unit => ({
      ...
      position: unit.position,
      weapon: unit.weapon,
      weaponEmoji: getWeapon(unit.weapon).emoji,
